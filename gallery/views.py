...
def get_pictures(request: HttpRequest, skip: int, limit: int):
    pictures = list(Picture.objects.all()[skip:skip+limit])
    pictures_answer = []
    ...
    for picture in pictures:
        ...
        pictures_answer.append(
            picture.__dict__ | {"reactions": reactions, "username": user.username}
        )

    return render(request, "main.html", {"pictures": pictures_answer})
